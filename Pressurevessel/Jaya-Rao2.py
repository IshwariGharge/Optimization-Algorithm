...
def Jayarao_2_updated_pressure_vessel():
    # Population size
    pop = 10
    # Number of design variables (x1, x2, x3, x4 for pressure vessel design)
    var = 4
    ...
    # Variable bounds
    mini = np.array([10, 10, 10, 10])  # Lower bounds for x1, x2, x3, x4
    maxi = np.array([100, 100, 100, 100])  # Upper bounds for x1, x2, x3, x4
    ...
    f = np.array([pressure_vessel(ind) for ind in x])
    gen = 1
    ...
    while gen <= max_gen:
        ...
        for _ in range(30):  # Repeat 30 times for each iteration
            # Update population using Rao-2 logic
            xnew = update_population(x, f)
            # Trim population within bounds
            xnew = trimr(mini, maxi, xnew)
            # Evaluate new population
            fnew = np.array([pressure_vessel(ind) for ind in xnew])
    ...
    df.to_excel("JayaRao2_Results_Pressure_Vessel_Updated_30Runs.xlsx", index=False)
    print("Results saved to 'JayaRao2_Results_Pressure_Vessel_Updated_30Runs.xlsx'.")
...
if __name__ == "__main__":
    Jayarao_2_updated_pressure_vessel()
