...
def Jayarao_3_updated_gear_train():
    # Population size
    pop = 10
    # Number of design variables (4 variables for the gear train)
    var = 4
    ...
    # Variable bounds (for gear train)
    mini = np.ones(var) * 0.1  # Example bounds for gear train
    maxi = np.ones(var) * 50   # Example bounds for gear train
    ...
    while gen <= max_gen:
        ...
        for _ in range(30):  # Repeat 30 times for each iteration
            # Update population using Rao-3 logic for gear train
            xnew = update_population_rao3(x, f)
    ...
    df.to_excel("JayaRao3_Results_GearTrain_Updated_30Runs.xlsx", index=False)
    print("Results saved to 'JayaRao3_Results_GearTrain_Updated_30Runs.xlsx'.")


def gear_train(solution):
    """Objective function for the gear train problem."""
...
def objective(x):
    """Objective function: Gear train function."""
    return np.apply_along_axis(gear_train, 1, x)


def update_population_rao3(x, f):
    """Update population using Rao-3 logic for gear train."""
    pop, var = x.shape
    best = x[np.argmin(f), :]
    worst = x[np.argmax(f), :]
