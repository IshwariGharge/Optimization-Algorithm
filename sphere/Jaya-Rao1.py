...
while run < Runs:
    ...
    for k in range(max_iter):
        ...
        for i in range(pop_size):
            for j in range(dim):
                r1 = random.random()
                Positions[i, j] = Positions[i, j] + r1 * (best_pos[j] - abs(Positions[i, j])) + r1 * (best_pos[j] - worst_pos[j])
                Positions[i, j] = np.clip(Positions[i, j], lb[j], ub[j])
...
# Write to Excel
with pd.ExcelWriter("Jaya-Rao1_Sphere_results.xlsx", engine='openpyxl') as writer:
    df_runs.to_excel(writer, sheet_name='Run_Results', index=False)
    df_summary.to_excel(writer, sheet_name='Summary', index=False)

print("\nResults saved to 'Jaya-Rao1_Sphere_results.xlsx'")
